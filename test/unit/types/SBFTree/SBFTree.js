...
describe('SBFTree', () => {
  ...
  it('should instantiate', async function () {
    ...
    expect(tree.adapter.leafs[bucket1Id].data.keys).to.deep.equal([0]);

    const bucket2Id = tree.root.childrens[0].childrens[1].id;
    expect(tree.adapter.leafs[bucket2Id].data.keys).to.deep.equal([10, 20]);
    ...
    expect(tree.adapter.leafs[bucket3Id].data.keys).to.deep.equal([30]);

    const bucket4Id = tree.root.childrens[1].childrens[1].id;
    expect(tree.adapter.leafs[bucket4Id].data.keys).to.deep.equal([40, 50]);
    ...
    expect(tree.adapter.leafs[bucket5Id].data.keys).to.deep.equal([60]);

    const bucket6Id = tree.root.childrens[2].childrens[1].id;
    expect(tree.adapter.leafs[bucket6Id].data.keys).to.deep.equal([70]);

    const bucket7Id = tree.root.childrens[2].childrens[2].id;
    expect(tree.adapter.leafs[bucket7Id].data.keys).to.deep.equal([80, 90]);
